...
Here is the algorithm:
    ...
    with the first, and if there are an odd number of digits, double every
    other digit starting with the second. Another way to think about it is,
    from the right to left, double every other digit starting with the second
    to last digit.
    ...
    If a resulting doubled number is greater than 9, replace it with either
    the sum of it's own digits, or 9 subtracted from it.
...
def validate(card_num):
    ...
    for i, digit in enumerate(card_num):
        if i % 2 == len(card_num) % 2:
            result.append(digit*2 if digit*2 < 10 else digit*2 - 9)
        else:
            result.append(digit)
